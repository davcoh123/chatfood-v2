...
const Onboarding = () => {
  ...
  const { settings, updateSettings } = useRestaurantSettings(user?.id);
  const { steps, progressPercent } = useOnboardingProgress(user?.id);
  ...
  // Local state for form data (simplified for now, ideally use react-hook-form per step)
  ...
  const handleSkip = async () => {
    // Just move to next step without saving (or save partial if needed)
    // For now, we assume skip means "do this later"
    if (currentStep < 5) {
      updateStepInUrl(currentStep + 1);
    } else {
      await updateSettings({ onboarding_completed: true });
