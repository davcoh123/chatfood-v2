  ...
  const fetchProfile = async (userId: string) => {
    try {
      ...
      if (profileError) {
        ...
        // Don't crash - set minimal profile
        setProfile({
          id: userId,
          user_id: userId,
          email: '',
      ...
      // Fetch role from user_roles table (with fallback)
      ...
      if (roleError) {
        console.warn('No role found for user, defaulting to "user":', roleError);
      }

      // Fetch plan from user_subscriptions table (with fallback)
      ...
      if (subscriptionError) {
        console.warn('No subscription found for user, defaulting to "starter":', subscriptionError);
      }
      ...
      // Set a minimal profile to avoid null issues
      setProfile({
        id: userId,
        user_id: userId,
        email: '',
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        // Ignore INITIAL_SESSION event as we handle it above
